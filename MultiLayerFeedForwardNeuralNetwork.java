...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		Delta[this.layers.length-1] = new double [outputs.length];
		// for each node j in the output layer do
		//     Delta[j] <- g'(in_j) \times (y_j - a_j)
		for(int j =0; j < outputs.length; j++){
			double in_j = outputs[j].getInputSum();
			double a_j = outputs[j].activation(in_j); //predicted
			double y_j = example.outputs[j]; //actual
			Delta[this.layers.length-1][j]= outputs[j].activationPrime(in_j) * (y_j-a_j);
		}
		// for l = L-1 to 1 do
		//     for each node i in layer l do 
		//         Delta[i] <- g'(in_i) * \sum_j w_ij Delta[j]
		for(int l = this.layers.length-2; l >=1;l--){
			Delta[l] = new double[this.layers[l].length];
			for(int i =0; i < this.layers[l].length; i++){
				double in_i = ((NeuronUnit) this.layers[l][i]).getInputSum();
				double d1 = ((NeuronUnit) this.layers[l][i]).activationPrime(in_i);
				double sum = 0;
				for(int j = 0; j < Delta[l+1].length; j++){
					List<Connection> outgoing = this.layers[l][i].outgoingConnections;
					sum += Delta[l+1][j] * outgoing.get(j).weight;
				}
				Delta[l][i] = d1*sum;
			}
		}
		...
		for(int i=0; i < this.layers.length-1; i++){
			for(int j = 0; j < this.layers[i].length; j++){
				List<Connection> ij = this.layers[i][j].outgoingConnections;
				for(int k = 0; k <ij.size(); k++){
					ij.get(k).weight += alpha * this.layers[i][j].getOutput() * Delta[i+1][k];
				}
			}
		}
	}
